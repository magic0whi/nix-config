{lib, ...}: {
  ...
  scanPaths = path: map (filename: path + "/${filename}") (builtins.attrNames (
    lib.filterAttrs (path: _type: (_type == "directory") # include directories
      || ((path != "default.nix") # ignore default.nix
        && (lib.hasSuffix ".nix" path)) # include .nix files
